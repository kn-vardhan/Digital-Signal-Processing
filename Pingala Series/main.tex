...
\end{align}
\solution
Download the python code for the problem from
\begin{lstlisting}
$ wget https://raw.githubusercontent.com/kn-vardhan/EE3900-Digital-Signal-Processing/main/pingala/codes/jee_2019.py
\end{lstlisting}
...
\\
\solution Download the python code for the stem plot from
\begin{lstlisting}
$ wget https://raw.githubusercontent.com/kn-vardhan/EE3900-Digital-Signal-Processing/main/pingala/codes/stem.py
\end{lstlisting}
and run it using the command
\begin{lstlisting}
python3 stem.py
...
\item Find $X^{+}(z)$.
\\
\solution Applying the one-sided $z-$transformation to \eqref{eq:10-pingala}, will give us
\begin{align}
    &\mathcal{Z}^+[x(n+2)] = \mathcal{Z}^+[x(n+1)]+ \mathcal{Z}^+[x(n)] \\
...
\item Find $x(n)$.
\\
\solution Expanding $X^+(z)$ from \eqref{eq:X-z}, gives
\begin{align}
...
\end{align}
\solution Download the Python code for sketch from
\begin{lstlisting}
$ wget https://raw.githubusercontent.com/kn-vardhan/EE3900-Digital-Signal-Processing/main/pingala/codes/y_z.py
\end{lstlisting}
and run it using the command
\begin{lstlisting}
python3 y_z.py
...
\item Find $Y^{+}(z)$. \\
\solution Applying the one-sided $z-$transformation to \eqref{eq:10-orig-diff-rev}, will give us
\begin{align}
\mathcal{Z}^+[y(n)] &= \mathcal{Z}^+[x(n-1)] + \mathcal{Z}^+[x(n+1)] \\
&= \frac{z + z^{-1}}{1 - z^{-1} - z^{-2}} - z
\end{align}
as $x(n) = 0 \: \forall \: n < 0$, therefore,
\begin{align}
Y^+(z) = \frac{1 + 2z^{-1}}{1 - z^{-1} - z^{-2}}, \quad |z| \ge \alpha
\end{align}
\item Find $y(n)$.
    \label{pr:1-3}
    \\
...
\end{align}
\solution From \eqref{eq:x-n-def}, and also as\\ $x(n) = 0\ \forall\ n < 0$, we get,
\begin{align}
...
\end{align}
\solution From \eqref{eq:x-n-def}, we can see that,
\begin{align}
a_{n+2} - 1 = [x\brak{n+1} - 1], \quad n \ge 0
\end{align}
Now, applying the property of $u(n)$ gives us,
\begin{align}
a_{n+1} - 1 = [x\brak{n+1} - 1]\:u(n), \quad n \ge 0
\end{align}
 \item Show that 
\begin{align}
...
\end{align}
We have already found the relevant answers of the options from \eqref{pr:1-3}, \eqref{pr:1-2}, and \eqref{pr:1-4}. 
\\
Therefore, \\
Correct Option(s) - $(a), (b), (c)$ \\
Incorrect Option(s) - $(d)$.
\end{enumerate}
